...
public class StreamQuestions {
	...
	private static void filterEvenNum(List<Integer> intList) {
		...
		Map<Boolean, List<Integer>> partitionEvenOdd = intList.stream().collect(Collectors.partitioningBy(i -> i%2 == 0));
	...
	private static void countStrStartWithLetter(List<String> strList, String startWith) {
		...
		List<String> strListStartWithIgnoreCase = strList.stream().filter(str -> str.toLowerCase().startsWith(startWith.toLowerCase())).toList();
